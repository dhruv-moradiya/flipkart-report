...
import { ReturnRecord, ParseResult } from "../features/returns/types/return.types";
...
import { ReportType, UploadedReportsState } from "../features/reports/types/report.types";
...
export function ExcelProvider({ children }: { children: React.ReactNode }) {
  const [activeReportType, setActiveReportType] = useState<ReportType | "journey" | null>(null);
  ...
  // Order Journey State
  const [selectedJourneyOrderId, setSelectedJourneyOrderId] = useState<string | null>(null);
  ...
  useEffect(() => {
    try {
      ...
      if (savedMeta) {
        ...
        setActiveReportType(meta.activeReportType || (meta.hasPnl ? "profit_loss" : "returns"));
      ...
      if (savedRecords) {
        ...
        const hydrated = parsedRecords.map((r) => ({
          ...r,
          returnRequestedDate: r.returnRequestedDate ? new Date(r.returnRequestedDate) : null,
          returnApprovalDate: r.returnApprovalDate ? new Date(r.returnApprovalDate) : null,
          completedDate: r.completedDate ? new Date(r.completedDate) : null,
          outForDeliveryDate: r.outForDeliveryDate ? new Date(r.outForDeliveryDate) : null,
          returnDeliveryPromiseDate: r.returnDeliveryPromiseDate ? new Date(r.returnDeliveryPromiseDate) : null,
  ...
  const setParseResult = (result: ParseResult | null) => {
    if (result) {
      ...
      try {
        ...
        sessionStorage.setItem(STORAGE_KEY_RETURNS, JSON.stringify(result.records));
      } catch {
